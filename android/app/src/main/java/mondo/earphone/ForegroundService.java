...
public class ForegroundService extends Service {
    ...
    public int andId_count; // 파베 UID 변경 (오류 제어를 위해)

    public static boolean current=false; // foreground가 현재 켜져 있는지 상태
    ...
    public void onCreate() {
        ...
        // 안드로이드 ID count 저장
        detectLog = getApplicationContext().getSharedPreferences("detectMode", Activity.MODE_PRIVATE);
        ...
        log_count = detectLog.getInt("detectLog_length", 0); // 가져왔는데 없으면 기본값 0
        ...
        //데이터 변화 감지중 (러닝결과 언제오나)
        reference.child("Result").child(appInfo.android_id+"-"+andId_count).addChildEventListener(new ChildEventListener() {
...
//        // 파이어베이스 삭제
        ...
        editor.putInt("android_id_count", ++andId_count); // log 숫자 추가하고 넣기
        editor.apply();
...
//        NotificationCompat.Action action = new NotificationCompat.Action.Builder(R.drawable.push_logo, getString(R.string.app_content)).build();
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, CHANNEL_ID)
                ...
                .setStyle(new NotificationCompat.BigPictureStyle()  // 푸쉬알림 밑에 화살표 터치시 보여줄 큰 사진 등록
                        ...
                        .bigLargeIcon(null))
//                .addAction(R.drawable.push_logo, getString(R.string.app_content), pendingIntent)
    ...
    private void awsS3Upload(File wavFile, String fileName){
        ...
        reference = FirebaseDatabase.getInstance().getReference().child("UID").child(appInfo.android_id+"-"+andId_count);   // 123은 나중에 실 휴대폰 고유 아이디 쓸예정   서버 반응하기 위해 데베 생성
    ...
    private void chatListener(DataSnapshot dataSnapshot) { // 러닝한 결과 가져다 주는곳
        ...
        switch (result){
            case"0":
                ...
                break;
            case"1":
                result="오토바이 소리"; // 엔진 소리
                bitmap =BitmapFactory.decodeResource(getResources(), R.drawable.cycle);
                break;
            case"2":
                result="경보기 소리";
                bitmap =BitmapFactory.decodeResource(getResources(), R.drawable.danger);
                break;
        }



        Log.d("감지 결과", "종류 : "+result+" ddddddddd");

        // 감지된게 있을시에만

//        || !result.equals("3")
        if(bitmap != null ) {
            NotificationSomethings(123, bitmap, result + "가 감지되었습니다");
            // 내부 저장소에 로그로 저장하기
            saveLog(now, result.split(" ")[0]);
        }
